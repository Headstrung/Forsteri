...
function reduceData(data::Array{Any,2}, header::Array{String,2}, remove::Array{Int64,1}=[-1])
    ...
    if length(missing) > 0 || length(ignore) > 0
        (dataNew, headerNew) = removeColumns(dataNew, vcat(missing, ignore),
            headerNew)
    end
...
function removeColumns(data::Array{Float64,2}, remove::Array{Int64,1}, header::Array{ASCIIString,1}=[""])
    #=
    Remove a list of columns from data.
