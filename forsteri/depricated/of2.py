...
class ManagerFrame(wx.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in range(0, 4):
            textLabels[i] = wx.StaticText(masterPanel, label=LABELS[i + 2])
    ...
    def updateList(self, event):
        """
        To Do:
          1) Implement the search functionality (possibly in another function).
        """

        """Get match implementation."""

        # Gett all of the products from the database.
        data = iface.getAllProducts()
        ...
        for i in range(1, 5):
            ...
            if selected != "":
                ...
                allValues = iface.getMatch(LABELS[i + 1])
    ...
    def onAdd(self, event):
        ...
        iface.addProduct(addProductData)
    ...
    def onEdit(self, event):
        ...
        # If nothing is input for product send an error and return false.
        if newProductData["Product"] is None:
            errorDialog = wx.MessageDialog(self,
                "Product cannot be empty.", "Error", wx.OK|wx.ICON_ERROR)
            errorDialog.ShowModal()
        ...
        # Add the inputted product data to the database.
        iface.setProduct(oldProductData[0], newProductData)
    ...
    def onEditMulti(self, count=-1):
        ...
        try:
            ...
            errorDialog = wx.MessageDialog(self,
                "Sku must be an integer.", "Error", wx.OK|wx.ICON_ERROR)
            errorDialog.ShowModal()
            return False
...
class InputDialog(wx.Dialog):
    ...
    def getTextEntry(self, missing=True):
        """
        This function should be surrounded with try, exception to catch bad
          Sku inputs.
        """
        ...
        # Iterate over the input objects and get the inputted values.
        index = 0
        for inputObject in self.inputObjects:
            entered = inputObject.GetValue()

            # If the index is one, the input was Sku and must be converted to
            # an integer. This conversion will raise a ValueError if entered
            # is not an integer.
            if index == 1:
                if entered == '':
                    inputs[LABELS[index]] = None
                else:
                    try:
                        inputs[LABELS[index]] = int(entered)
                    except ValueError:
                        inputs[LABELS[index]] = None
            else:
                if entered == '' and missing:
                    inputs[LABELS[index]] = None
                elif entered == '':
                    index += 1
                    continue
    ...
    def setTextEntry(self, text):
        ...
        index = 0
        for inputObject in self.inputObjects:
            inputObject.SetValue(str(text[index]))
            index += 1
